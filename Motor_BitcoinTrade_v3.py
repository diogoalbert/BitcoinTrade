...
def processar_motor_final_triplo(file_path):
    ...
    for ts, group in df.groupby('Timestamp'):
        data_atual = ts
        data_s = data_atual.strftime('%Y-%m-%d')
        hora_s = data_atual.strftime('%H:%M:%S')
        ...
        for _, row in entradas.iterrows():
            ...
            custo_total = 0.0
            origem_ext = "Sim" if "Depósito" in row['Categoria'] else "Não"
    ...
    # Salvar
    pd.DataFrame(log_irs).to_csv('Arquivo1_IRS.csv', index=False,
